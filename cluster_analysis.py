...
def add_p_value_brackets(ax, x1, x2, y, p_value, bracket_height=0.02, text_y_offset=0.1, fontsize=14):
    ...
    ax.text((x1 + x2) / 2, y + bracket_height - text_y_offset, p_text, ha='center', va='bottom', color='black', fontsize=fontsize)
...
def plot_data_with_p_values(ax, column_index, ylabel, is_first_plot, is_last_plot, label_text, condition_column_name, condition_value, total_score_condition, y_coordinates):
    ...
    for file_name, label, position in zip(file_names, cluster_names, tick_positions):
        ...
        color_mapping = {'2': 'grey', '4': 'cyan', '1': 'forestgreen', '5': 'magenta'}
    ...
    ax.text(0.999, 0.009, label_text, transform=ax.transAxes, fontsize=16, fontweight='bold', ha='right', va='bottom')
    ...
    upper_ax.set_xticklabels([f"{load_data(file_name, column_index, condition_column_name, condition_value, total_score_condition).count()}" for file_name in file_names], fontsize=16, ha='center', rotation=0)
    ...
    ax.set_xlabel('Cluster', fontsize=20, fontweight='bold', labelpad=10)  # Add x-axis label to the current subplot
...
specified_y_coordinates_plot1 = [-618, -610, -600, -590, -580, -570]
specified_y_coordinates_plot2 = [1, 4, 8, 12, 16, 20]
specified_y_coordinates_plot3 = [1800, 1900, 2000, 2100, 2200, 2300]
